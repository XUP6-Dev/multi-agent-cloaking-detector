...
| Provider | 說明 |
|---|---|
| `deepseek`（預設） | 雲端，需要 `DEEPSEEK_API_KEY` |
| `anthropic` | 雲端，需要 `ANTHROPIC_API_KEY` |
| `openai` | 雲端，需要 `OPENAI_API_KEY` |
| `google` | 雲端，需要 `GOOGLE_API_KEY` |
| `lmstudio` | 本地，免 key（先在 LM Studio 開 Local Server），`LLM_MODEL` 需填 Local Server 分頁顯示的確切 model identifier |

切換：`$env:LLM_PROVIDER = "lmstudio"`（PowerShell）。
...
- [v20260719_Gary_reviewedPhishCloak.md](v20260719_Gary_reviewedPhishCloak.md) — 對應的學術論文寫法（形式化定義、實驗方法論；Section 5 實驗數據待補新資料）
